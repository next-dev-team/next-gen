...
import { prepareMessages, formatMessagesAsPrompt } from '../memory.js';
...
export async function runCodex(prompt: string, model?: string): Promise<string> {
  ...
  return await new Promise((resolve, reject) => {
    ...
    child.on('error', (err) => {
      ...
      reject(
        new Error(`Codex command failed: ${err.message}. Make sure 'codex' CLI is installed.`),
      );
    });
    ...
    child.on('close', (code, signal) => {
      ...
      if (code !== 0) {
        ...
        reject(
          new Error(`Codex failed (exit ${code}${signalInfo}): ${stderrText || 'no error output'}`),
        );
        return;
      }
 ...
 */
export async function handleCodex(ctx: ProviderContext): Promise<ProviderResult> {
  ...
  const lastMessage = optimizedMessages.filter((m) => m.role !== 'system').pop();
