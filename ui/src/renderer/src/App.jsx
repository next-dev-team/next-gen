...
function App() {
  ...
  const handleGeneratorSelect = (gen) => {
    ...
    gen.prompts.forEach(p => {
        if (p.default !== undefined && typeof p.default !== 'function') initialAnswers[p.name] = p.default
        if (p.type === 'checkbox') initialAnswers[p.name] = []
        if (p.type === 'list' && p.choices && p.choices.length > 0) initialAnswers[p.name] = p.choices[0].value
    })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        if (window.electronAPI) {
            const result = await window.electronAPI.runGenerator(selectedGenerator.name, answers)
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <h1 className="text-3xl font-bold mb-8 text-blue-400">Next Gen Generator</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
                {generators.map(gen => (
                    <button
                        ...
                        className={`w-full text-left p-3 rounded transition-colors ${selectedGenerator?.name === gen.name ? 'bg-blue-600 text-white' : 'bg-gray-700 hover:bg-gray-600'}`}
                    >
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-semibold mb-4">
                {selectedGenerator ? `Configure ${selectedGenerator.name}` : 'Select a Generator'}
            </h2>
            ...
            {selectedGenerator ? (
                                ...
                                <div className="space-y-2">
                                    {prompt.choices.map(c => (
                                        <label key={c.value} className="flex items-center space-x-2">
                                            <input
                                                type="checkbox"
                                                checked={(answers[prompt.name] || []).includes(c.value)}
                                                onChange={e => handleCheckboxChange(prompt.name, c.value, e.target.checked)}
                ...
                <p className="text-gray-400">Please select a generator from the list to start.</p>
            )}
        </div>
      </div>
